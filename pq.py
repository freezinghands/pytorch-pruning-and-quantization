...
def main_worker(gpu, ngpus_per_node, args):
    ...
    iter_amount, tune_batch_num = args.prune_step, 10
    iter_cnt = math.ceil(prune_amount * 100 / iter_amount)
    for epoch in range(0, iter_cnt):
        if args.distributed:
            train_sampler.set_epoch(epoch)
        print(f"=> pruning iter: {(1 + epoch):2d}/{iter_cnt:2d}")
        prune_all_layers(model, prune_amount=iter_amount/100)
        # split train dataset - only 2 percent
        split_amt = math.ceil(len(train_dataset) * 0.02)
        tuning_dataset, _ = random_split(train_dataset, [split_amt, len(train_dataset) - split_amt])
        print(f'=>   fine tuning with {math.ceil(split_amt/args.batch_size)} batches')
        tuning_loader = torch.utils.data.DataLoader(
            tuning_dataset, batch_size=args.batch_size, shuffle=(train_sampler is None),
            num_workers=args.workers, pin_memory=True, sampler=train_sampler)
        # train for one epoch
        train(tuning_loader, model, criterion, optimizer, epoch, args)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                and args.rank % ngpus_per_node == 0) and epoch is (iter_cnt - 1):
            save_checkpoint({
                'epoch': epoch + 1,
                'arch': args.arch,
            ...
            }, False, filename=f'checkpoint_{int(prune_amount*100)}.pth.tar', 
            bestfilename=f'model_best_prune_{int(prune_amount*100)}.pth.tar')

    def remove_prune_all_layers(module: torch.nn.Module):
        for sub_idx, sub_module in module._modules.items():
            if isinstance(sub_module, torch.nn.Conv2d):
                ...
                remove_prune_all_layers(sub_module)
    remove_prune_all_layers(model)
    print("=> pruning complete")
    # Save state_dict without quantization
    os.makedirs("saved_model", exist_ok=True)
    state_dict_filename = f"{args.arch}_{int(prune_amount*100)}_b{args.batch_size}_step{args.prune_step}_statedict.pth"
    torch.save(model.state_dict(), os.path.join("saved_model", state_dict_filename))
    print(f"=> state_dict without quantization saved in saved_model directory, as {state_dict_filename}")
    print("=> validation for checking accuracy")
    validate(val_loader, model, criterion, args)
    print("=> valiation complete")

    # Skipping quantization - need to be deleted if want to quantize
    print("=> exiting pq.py...")
    return
    ...
    def calibrate(model, data_loader):         # calibration function
        ...
        with torch.no_grad():                  # do not save gradient when evaluation mode
            for image, target in data_loader:  # extract input and output data
                ...
                print(f'\r=>   calibration iter: {cnt:3d}/{len(data_loader):3d}', end='')
                cnt += 1

    model_prepared = quantize_fx.prepare_fx(model, qconfig_dict)  # preparation
    if not args.do_not_calibrate:
        calibrate(model_prepared, val_loader)                    # calibration
    model_quantized = quantize_fx.convert_fx(model_prepared)      # convert the model
    print("=> quantization completed")
    save_checkpoint({
        'epoch': args.start_epoch + iter_cnt + 1,
        'arch': args.arch,
        'state_dict': model.state_dict(),
        'best_acc1': best_acc1,
        'optimizer' : optimizer.state_dict(),
        'scheduler' : scheduler.state_dict()
    }, False, filename=f"{args.arch}_{quant_type}quant_{int(prune_amount*100)}.pth.tar")
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar', bestfilename='model_best.pth.tar'):
